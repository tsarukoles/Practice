...
describe("Search listings - Listings Page", () => {
  before(() => {
    cy.apiLogin(credentials.realtor.email, credentials.realtor.password);
    cy.createListing("images/listing-picture.jpg", verificationText).then((responseBody) => {
  ...
  beforeEach(() => {
    ...
    featuredListingsPage.visitBasedOnPrice(searchByItName, priceRange);
    featuredListingsPage.darkModeListingCheck.check();
  });
  after("Delete", () => {
    if (listingId) {
  ...
  it("Should search by keyword", () => {
    featuredListingsPage.searchListingName.type(verificationText.keyword);
    cy.contains('Start Search').click();
    featuredListingsPage.listingTitle.should('contain', verificationText.title);
  });

  it("Should search by bedrooms", () => {
    featuredListingsPage.searchBedrooms.click();
    featuredListingsPage.selectBedroomsNumber(2); 
    cy.contains('Start Search').click();
  ...
  it("Should search by city", () => {
    featuredListingsPage.searchListingCity.type(verificationText.city);
    cy.contains('Start Search').click();
